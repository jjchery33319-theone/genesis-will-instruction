...
import { Gift, Plus, Trash2, Heart, Copy, RotateCcw } from "lucide-react";
    ...
    <div className="space-y-5">
      ...
      >
        ...
        {isMirrorWill && (() => {
          ...
          const { hasSnapshot: hasGiftSnapshot, saveSnapshot: saveGiftSnapshot, undo: undoGifts } = useCopyUndo(data, onChange);
          const copyGiftsFromClient1 = () => {
            saveGiftSnapshot(["client2SpecificGifts"]);
            ...
            <>
              {hasGiftSnapshot && (
                <div className="flex items-center gap-2 p-2.5 rounded-lg mb-3" style={{ background: "oklch(0.98 0.02 85)", border: "1px solid oklch(0.78 0.12 85 / 0.5)" }}>
                  ...
                  <span className="text-xs flex-1" style={{ color: "oklch(0.35 0.08 85)" }}>Gifts copied from Client 1. Changed your mind?</span>
                  <Button type="button" variant="outline" size="sm" className="text-xs gap-1.5" style={{ borderColor: "oklch(0.78 0.12 85 / 0.6)", color: "oklch(0.45 0.1 85)" }} onClick={undoGifts}>
