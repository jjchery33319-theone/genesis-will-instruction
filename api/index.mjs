...
import { eq as eq4, and, desc as desc2, sql } from "drizzle-orm";
async function ensureClientNameColumns(db) {
  if (_migrated) return;
  _migrated = true;
  try {
...
async function enrichMatter(matter) {
  ...
  const [clients, executors, guardians, beneficiaries, wishes, gifts, pets, properties, businesses, trustClauses, exclusions, lettersOfWishes] = await Promise.all([
    ...
    db.select().from(matterLettersOfWishes).where(eq4(matterLettersOfWishes.matterId, matter.id))
  ]);
...
var mattersRouter = router({
  ...
  transferFromInstruction: protectedProcedure.input(z4.object({ instructionId: z4.number().int() })).mutation(async ({ input }) => {
    ...
    await upsertClient(matterId, "testator1", {
      title: ins.client1Prefix ?? void 0,
      firstName: ins.client1FirstName ?? void 0,
      middleName: ins.client1MiddleName ?? void 0,
      lastName: ins.client1LastName ?? void 0,
      fullName: c1Name,
      address: c1Address,
    ...
    if (isMirror) {
      ...
      await upsertClient(matterId, "testator2", {
        title: ins.client2Prefix ?? void 0,
        firstName: ins.client2FirstName ?? void 0,
        middleName: ins.client2MiddleName ?? void 0,
        lastName: ins.client2LastName ?? void 0,
        fullName: c2Name,
        address: c2Address,
...
function generateWelcomePackHtml(record) {
  ...
  function completeRecordSection(title, fields) {
    ...
    return `${sectionHeading2(iconCheck, title)}<div class="info-cards-row">${presentFields.map(([label, value]) => infoCard(label, escapeRecordValue(recordValue(value)))).join("")}</div>`;
  }
  const asArray = (value) => {
    const parsed = parseStoredValue2(value);
    return Array.isArray(parsed) ? parsed : [];
  ...
  const allEnteredFields = Object.entries(record).filter(([key, value]) => !welcomePackExcludedFields.has(key) && hasStoredValue(value)).map(([key, value]) => [
    humaniseKey(key),
